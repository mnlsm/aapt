...
status_t ZipEntry::initFromCDE(FILE* fp)
{
    ...
    if (fseek(fp, mCDE.mLocalHeaderRelOffset, SEEK_SET) != 0) {
        printf("local header seek failed (%ld)\n",
...
bool ZipEntry::compareHeaders(void) const
{
    if (mCDE.mVersionToExtract != mLFH.mVersionToExtract) {
        printf("cmp: VersionToExtract\n");
        return false;
    }
    if (mCDE.mGPBitFlag != mLFH.mGPBitFlag) {
        printf("cmp: GPBitFlag\n");
        return false;
    }
    if (mCDE.mCompressionMethod != mLFH.mCompressionMethod) {
        printf("cmp: CompressionMethod\n");
        return false;
    }
    if (mCDE.mLastModFileTime != mLFH.mLastModFileTime) {
        printf("cmp: LastModFileTime\n");
        return false;
    }
    if (mCDE.mLastModFileDate != mLFH.mLastModFileDate) {
        printf("cmp: LastModFileDate\n");
        return false;
    }
    ...
    if (mCDE.mCompressedSize != mLFH.mCompressedSize) {
        printf("cmp: CompressedSize\n");
        return false;
    }
    if (mCDE.mUncompressedSize != mLFH.mUncompressedSize) {
        printf("cmp: UncompressedSize\n");
        return false;
    }
    if (mCDE.mFileNameLength != mLFH.mFileNameLength) {
        printf("cmp: FileNameLength\n");
        return false;
    }
#if 0       // this seems to be used for padding, not real data
    if (mCDE.mExtraFieldLength != mLFH.mExtraFieldLength) {
        printf("cmp: ExtraFieldLength\n");
        return false;
    }
#endif
...
status_t ZipEntry::LocalFileHeader::read(FILE* fp)
{
    ...
    if (ZipEntry::getLongLE(&buf[0x00]) != kSignature) {
        printf("whoops: didn't find expected signature\n");
        result = UNKNOWN_ERROR;
        goto bail;
    }
...
{
    printf(" LocalFileHeader contents:\n");
    printf("  versToExt=%u gpBits=0x%04x compression=%u\n",
        mVersionToExtract, mGPBitFlag, mCompressionMethod);
    printf("  modTime=0x%04x modDate=0x%04x crc32=0x%08lx\n",
        mLastModFileTime, mLastModFileDate, mCRC32);
    printf("  compressedSize=%lu uncompressedSize=%lu\n",
        mCompressedSize, mUncompressedSize);
    printf("  filenameLen=%u extraLen=%u\n",
...
{
    printf(" CentralDirEntry contents:\n");
    printf("  versMadeBy=%u versToExt=%u gpBits=0x%04x compression=%u\n",
        mVersionMadeBy, mVersionToExtract, mGPBitFlag, mCompressionMethod);
    printf("  modTime=0x%04x modDate=0x%04x crc32=0x%08lx\n",
        mLastModFileTime, mLastModFileDate, mCRC32);
    printf("  compressedSize=%lu uncompressedSize=%lu\n",
        mCompressedSize, mUncompressedSize);
    printf("  filenameLen=%u extraLen=%u commentLen=%u\n",
        mFileNameLength, mExtraFieldLength, mFileCommentLength);
    printf("  diskNumStart=%u intAttr=0x%04x extAttr=0x%08lx relOffset=%lu\n",
