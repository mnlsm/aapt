...
struct str_parms *str_parms_create_str(const char *_string)
{
    ...
    printf("%s: source string == '%s'\n", __FUNCTION__, _string);

	kvpair = strtok_s(str, ";", &tmpstr);
    while (kvpair && *kvpair) {
...
int str_parms_add_int(struct str_parms *str_parms, const char *key, int value)
{
	...
	ret = sprintf_s(val_str, sizeof(val_str), "%d", value);
    if (ret < 0)
        return -EINVAL;
                        ...
                        float value)
{
    ...
    ret = sprintf_s(val_str, sizeof(val_str), "%.10f", value);
    if (ret < 0)
        return -EINVAL;
...
static bool combine_strings(void *key, void *value, void *context)
{
	...
	char *new_str = (char*)malloc((*old_str ? strlen(*old_str) + 1 : 0)
		+ strlen((char*)key) + strlen((char*)value) + 1);
	int ret = sprintf_s(new_str, sizeof(new_str), "%s%s%s=%s",
                   *old_str ? *old_str : "",
                   *old_str ? ";" : "",
                   (char *)key,
                   (char *)value);
