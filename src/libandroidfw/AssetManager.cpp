...
AssetManager::AssetManager() :
    ...
    if (kIsDebug) {
        printf("Creating AssetManager %p #%d\n", this, count);
    }
...
AssetManager::~AssetManager() {
    ...
    if (kIsDebug) {
        printf("Destroying AssetManager in %p #%d\n", this, count);
    }
...
bool AssetManager::addAssetPath(
    ...
    if (ap.type == kFileTypeRegular) {
        ...
        if (ap.type != kFileTypeDirectory && ap.type != kFileTypeRegular) {
            printf("Asset path %s is neither a directory nor file (type=%d).",
    ...
    printf("In %p Asset %s path: %s", this,
...
bool AssetManager::addOverlayPath(const String8& packagePath, int32_t* cookie)
{
    ...
    if ((idmap = openAssetFromFileLocked(idmapPath, Asset::ACCESS_BUFFER)) == NULL) {
        printf("failed to open idmap file %s\n", idmapPath.string());
        return false;
    }
    ...
    if (!ResTable::getIdmapInfo(idmap->getBuffer(false), idmap->getLength(),
        ...
        printf("failed to read idmap file %s\n", idmapPath.string());
        delete idmap;
        return false;
    }
    delete idmap;
    ...
    if (overlayPath != packagePath) {
        printf("idmap file %s inconcistent: expected path %s does not match actual path %s\n",
    ...
    oap.idmap = idmapPath;
#if 0
    printf("Overlay added: targetPath=%s overlayPath=%s idmapPath=%s\n",
        ...
        uint32_t targetCrc, uint32_t overlayCrc, uint32_t** outData, size_t* outSize)
{
    ...
    bool ret = false;
    {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (assets[i] == NULL) {
                printf("failed to find resources.arsc in %s\n", ap.path.string());
                goto exit;
            }
            if (tables[i].add(assets[i]) != NO_ERROR) {
                printf("failed to add %s to resource table", paths[i].string());
                goto exit;
            }
        }
...
Asset* AssetManager::open(const char* fileName, AccessMode mode)
{
    ...
    while (i > 0) {
        i--;
        printf("Looking for asset '%s' in '%s'\n",
...
Asset* AssetManager::openNonAsset(const char* fileName, AccessMode mode, int32_t* outCookie)
{
    ...
    while (i > 0) {
        i--;
        printf("Looking for non-asset '%s' in '%s'\n", fileName, mAssetPaths.itemAt(i).path.string());
...
bool AssetManager::appendPathToResTable(const asset_path& ap, bool appAsLib) const {
    ...
    printf("Looking for resource asset in '%s'\n", ap.path.string());
    if (ap.type != kFileTypeDirectory) {
        ...
        if (sharedRes == NULL) {
            ...
            if (ass == NULL) {
                printf("loading resource table %s\n", ap.path.string());
                ass = const_cast<AssetManager*>(this)->
            ...
            if (nextEntryIdx == 0 && ass != NULL) {
                ...
                printf("Creating shared resources for %s", ap.path.string());
    ...
    if ((ass != NULL || sharedRes != NULL) && ass != kExcludedAsset) {
        printf("Installing resource asset %p in to table %p\n", ass, mResources);
        if (sharedRes != NULL) {
            printf("Copying existing resources for %s", ap.path.string());
            mResources->add(sharedRes, ap.isSystemAsset);
        } else {
            printf("Parsing resources for %s", ap.path.string());
        ...
        printf("Installing empty resources in to table %p\n", mResources);
...
const ResTable* AssetManager::getResTable(bool required) const
{
    ...
    if (required && onlyEmptyResources) {
        printf("Unable to find resources file resources.arsc");
        delete mResources;
        mResources = NULL;
    }
...
Asset* AssetManager::openIdmapLocked(const struct asset_path& ap) const
{
    ...
    if (ap.idmap.size() != 0) {
        ...
        if (ass) {
            printf("loading idmap %s\n", ap.idmap.string());
        } else {
            printf("failed to load idmap %s\n", ap.idmap.string());
        }
    }
    return ass;
}
    ...
    const ZipEntryRO entry, AccessMode mode, const String8& entryName)
{
    ...
    if (!pZipFile->getEntryInfo(entry, &method, &uncompressedLen, NULL, NULL,
            NULL, NULL))
    {
        printf("getEntryInfo failed\n");
        return NULL;
    }

    FileMap* dataMap = pZipFile->createEntryFileMap(entry);
    if (dataMap == NULL) {
        printf("create map from entry failed\n");
        return NULL;
    }

    if (method == ZipFileRO::kCompressStored) {
        pAsset = Asset::createFromUncompressedMap(dataMap, mode);
        printf("Opened uncompressed entry %s in zip %s mode %d: %p", entryName.string(),
                dataMap->getFileName(), mode, pAsset);
    } else {
        pAsset = Asset::createFromCompressedMap(dataMap,
            static_cast<size_t>(uncompressedLen), mode);
        printf("Opened compressed entry %s in zip %s mode %d: %p", entryName.string(),
                dataMap->getFileName(), mode, pAsset);
    }
    if (pAsset == NULL) {
        /* unexpected */
        printf("create from segment failed\n");
    }
...
AssetDir* AssetManager::openDir(const char* dirName)
{
    ...
    while (i > 0) {
        ...
        if (ap.type == kFileTypeRegular) {
            printf("Adding directory %s from zip %s", dirName, ap.path.string());
            scanAndMergeZipLocked(pMergedInfo, ap, kAssetsRoot, dirName);
        } else {
            printf("Adding directory %s from dir %s", dirName, ap.path.string());
    ...
    const asset_path& ap, const char* rootDir, const char* baseDirName)
{
    ...
    if (!pZip->startIteration(&iterationCookie, dirName.string(), NULL)) {
        printf("ZipFileRO::startIteration returned false");
        return false;
    }
...
{
    AutoMutex _l(gLock);
    printf("Getting from SharedZip %p resource asset %p\n", this, mResourceTableAsset);
...
{
    printf("Getting from SharedZip %p resource table %p\n", this, mResourceTable);
...
{
    if (kIsDebug) {
        printf("Destroying SharedZip %p %s\n", this, (const char*)mPath);
    }
    if (mResourceTable != NULL) {
