...
bool ResolveAttrs(ResTable::Theme* theme, uint32_t def_style_attr,
  ...
  if (kDebugStyles) {
    printf("APPLY STYLE: theme=0x%p defStyleAttr=0x%x defStyleRes=0x%x", theme,
  ...
  for (size_t ii = 0; ii < attrs_length; ii++) {
    ...
    if (kDebugStyles) {
      printf("RETRIEVING ATTR 0x%08x...", cur_ident);
    }
    ...
    if (src_values_length > 0 && src_values[ii] != 0) {
      ...
      if (kDebugStyles) {
        printf("-> From values: type=0x%x, data=0x%08x", value.dataType,
              value.data);
      }
    } else {
      ...
      if (def_style_entry != def_style_end) {
        ...
        if (kDebugStyles) {
          printf("-> From def style: type=0x%x, data=0x%08x", value.dataType, value.data);
        }
      }
    }
    ...
    if (value.dataType != Res_value::TYPE_NULL) {
      ...
      if (kDebugStyles) {
        printf("-> Resolved attr: type=0x%x, data=0x%08x", value.dataType, value.data);
      }
      ...
      if (new_block >= 0) {
        if (kDebugStyles) {
          printf("-> From theme: type=0x%x, data=0x%08x", value.dataType, value.data);
        }
        new_block = res.resolveReference(&value, new_block, &resid, &type_set_flags, &config);
        if (new_block >= 0) block = new_block;
        if (kDebugStyles) {
          printf("-> Resolved theme: type=0x%x, data=0x%08x", value.dataType, value.data);
        }
      }
    }
    ...
    if (kDebugStyles) {
      printf("Attribute 0x%08x: type=0x%x, data=0x%08x", cur_ident, value.dataType, value.data);
    }
...
void ApplyStyle(ResTable::Theme* theme, ResXMLParser* xml_parser, uint32_t def_style_attr,
  ...
  if (kDebugStyles) {
    printf("APPLY STYLE: theme=0x%p defStyleAttr=0x%x defStyleRes=0x%x xml=0x%p",
  ...
  for (size_t ii = 0; ii < attrs_length; ii++) {
    ...
    if (xml_attr_idx != xml_attr_end) {
      ...
      if (kDebugStyles) {
        printf("-> From XML: type=0x%x, data=0x%08x", value.dataType, value.data);
      }
    }
    ...
    if (value.dataType == Res_value::TYPE_NULL && value.data != Res_value::DATA_NULL_EMPTY) {
      ...
      if (style_attr_entry != style_attr_end) {
        ...
        if (kDebugStyles) {
          printf("-> From style: type=0x%x, data=0x%08x", value.dataType, value.data);
        }
      }
    }
