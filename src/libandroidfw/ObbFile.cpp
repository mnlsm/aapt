...
bool ObbFile::readFrom(const char* filename)
{
    ...
    if (fd < 0) {
        printf("couldn't open file %s: %s", filename, strerror(errno));
        goto out;
    }
    success = readFrom(fd);
    close(fd);
    ...
    if (!success) {
        printf("failed to read from %s (fd=%d)\n", filename, fd);
    }
...
{
    if (fd < 0) {
        printf("attempt to read from invalid fd\n");
        return false;
    }
...
{
	int64_t fileLength = _lseeki64(fd, 0, SEEK_END);

    if (fileLength < kFooterMinSize) {
        if (fileLength < 0) {
            printf("error seeking in ObbFile: %s\n", strerror(errno));
        } else {
            printf("file is only %lld (less than %d minimum)\n", (long long int)fileLength, kFooterMinSize);
        }
        return false;
    }
    ...
    {
        _lseeki64(fd, fileLength - kFooterTagSize, SEEK_SET);

        char footer[kFooterTagSize];
        actual = read(fd, footer, kFooterTagSize);
        if (actual != kFooterTagSize) {
            printf("couldn't read footer signature: %s\n", strerror(errno));
            return false;
        }

        unsigned int fileSig = get4LE((unsigned char*)footer + sizeof(int32_t));
        if (fileSig != kSignature) {
            printf("footer didn't match magic string (expected 0x%08x; got 0x%08x)\n",
                    kSignature, fileSig);
            return false;
        }
        ...
        if (footerSize > (size_t)fileLength - kFooterTagSize
            ...
            printf("claimed footer size is too large (0x%08zx; file size is 0x%08lld)\n",
                    footerSize, (long long int)fileLength);
            return false;
        }

        if (footerSize < (kFooterMinSize - kFooterTagSize)) {
            printf("claimed footer size is too small (0x%zx; minimum size is 0x%x)\n",
                    footerSize, kFooterMinSize - kFooterTagSize);
            return false;
        }
    }

    int64_t fileOffset = fileLength - footerSize - kFooterTagSize;
	if (_lseeki64(fd, fileOffset, SEEK_SET) != fileOffset) {
        printf("seek %lld failed: %s\n", (long long int)fileOffset, strerror(errno));
        return false;
    }
    ...
    if (scanBuf == NULL) {
        printf("couldn't allocate scanBuf: %s\n", strerror(errno));
        return false;
    }

    actual = read(fd, scanBuf, footerSize);
    // readAmount is guaranteed to be less than kMaxBufSize
    if (actual != (ssize_t)footerSize) {
        printf("couldn't read ObbFile footer: %s\n", strerror(errno));
        free(scanBuf);
        return false;
    }
    ...
    if (sigVersion != kSigVersion) {
        printf("Unsupported ObbFile version %d\n", sigVersion);
        free(scanBuf);
        return false;
    }
    ...
    if (packageNameLen == 0
        ...
        printf("bad ObbFile package name length (0x%04zx; 0x%04zx possible)\n",
                packageNameLen, footerSize - kPackageNameOffset);
        free(scanBuf);
        return false;
    }
...
#ifdef DEBUG
    printf("Obb scan succeeded: packageName=%s, version=%d\n", mPackageName.string(), mVersion);
#endif
...
bool ObbFile::writeTo(int fd)
{
    ...
    if (mPackageName.size() == 0 || mVersion == -1) {
        printf("tried to write uninitialized ObbFile data\n");
        return false;
    }
    ...
    if (write(fd, &intBuf, sizeof(uint32_t)) != (ssize_t)sizeof(uint32_t)) {
        printf("couldn't write signature version: %s\n", strerror(errno));
        return false;
    }

    put4LE(intBuf, mVersion);
    if (write(fd, &intBuf, sizeof(uint32_t)) != (ssize_t)sizeof(uint32_t)) {
        printf("couldn't write package version\n");
        return false;
    }
    ...
    if (write(fd, &intBuf, sizeof(uint32_t)) != (ssize_t)sizeof(uint32_t)) {
        printf("couldn't write package name length: %s\n", strerror(errno));
        return false;
    }

    if (write(fd, mPackageName.string(), packageNameLen) != (ssize_t)packageNameLen) {
        printf("couldn't write package name: %s\n", strerror(errno));
        return false;
    }

    put4LE(intBuf, kPackageNameOffset + packageNameLen);
    if (write(fd, &intBuf, sizeof(uint32_t)) != (ssize_t)sizeof(uint32_t)) {
        printf("couldn't write footer size: %s\n", strerror(errno));
        return false;
    }

    put4LE(intBuf, kSignature);
    if (write(fd, &intBuf, sizeof(uint32_t)) != (ssize_t)sizeof(uint32_t)) {
        printf("couldn't write footer magic signature: %s\n", strerror(errno));
        return false;
    }
...
out:
    if (!success) {
        printf("failed to remove signature from %s: %s\n", filename, strerror(errno));
    }
    return success;
}
...
bool ObbFile::removeFrom(int fd)
{
	...
	SetFilePointer(openfile_handle, mFooterStart, NULL, FILE_BEGIN);
