...
/* static */ ZipFileRO* ZipFileRO::open(const char* zipFileName)
{
    ...
    if (error) {
		printf("Error opening archive %s: %s", zipFileName, ErrorCodeString(error));
        CloseArchive(handle);
        return NULL;
    }
...
bool ZipFileRO::startIteration(void** cookie, const char* prefix, const char* suffix)
{
    ...
    int32_t error = StartIteration(mHandle, &(ze->cookie),
                                   ...
                                   suffix ? &se : NULL);
    if (error) {
		printf("Could not start iteration over %s: %s", mFileName, ErrorCodeString(error));
        delete ze;
        return false;
    }
...
ZipEntryRO ZipFileRO::nextEntry(void* cookie)
{
    ...
    if (error) {
        if (error != -1) {
			printf("Error iteration over %s: %s", mFileName, ErrorCodeString(error));
        }
        return NULL;
    }
    ...
    const
{
    ...
    if (bufLen < requiredSize) {
		printf("Buffer too short, requires %d bytes for entry name", requiredSize);
        return requiredSize;
    }
...
bool ZipFileRO::uncompressEntry(ZipEntryRO entry, void* buffer, size_t size) const
{
    ...
    const int32_t error = ExtractToMemory(mHandle, &(zipEntry->entry),
        (uint8_t*) buffer, size);
    if (error) {
		printf("ExtractToMemory failed with %s", ErrorCodeString(error));
        return false;
    }
