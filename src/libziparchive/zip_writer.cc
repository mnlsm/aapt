...
ZipWriter::ZipWriter(FILE* f)
  ...
  if (fstat(_fileno(f), &file_stats) == 0) {
...
int32_t ZipWriter::DiscardLastEntry() {
  ...
  if (_fseeki64(file_, current_offset_, SEEK_SET) != 0) {
    return HandleError(kIoError);
  }
  files_.pop_back();
  return kNoError;
...
int32_t ZipWriter::FinishEntry() {
  ...
  if ((current_file_entry_.compression_method & kCompressDeflated) || !seekable_) {
    ...
    // Seek back to the header and rewrite to include the size.
	  if (_fseeki64(file_, current_file_entry_.local_file_header_offset, SEEK_SET) != 0) {
      return HandleError(kIoError);
    }
...
int32_t ZipWriter::Finish() {
  ...
  HANDLE openfile_handle = (HANDLE)::_get_osfhandle(_fileno(file_));
  SetFilePointer(openfile_handle, current_offset_, NULL, FILE_BEGIN);
  ::SetEndOfFile(openfile_handle);
