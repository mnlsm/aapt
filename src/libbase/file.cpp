...
#include "android-base/macros.h"  // For  on Darwin.
...
bool WriteStringToFile(const std::string& content, const std::string& path,
  ...
  android::base::unique_fd fd((open(path.c_str(), flags, mode)));
  if (fd == -1) {
...
bool WriteStringToFile(const std::string& content, const std::string& path,
  ...
  int flags = O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_BINARY |
              (follow_symlinks ? 0 : O_NOFOLLOW);
  android::base::unique_fd fd((open(path.c_str(), flags, 0666)));
  if (fd == -1) {
    return false;
  }
...
static int pread(int fd, void* data, size_t byte_count, int64_t offset) {
  DWORD bytes_read;
...
bool ReadFullyAtOffset(int fd, void* data, size_t byte_count, int64_t offset) {
  uint8_t* p = reinterpret_cast<uint8_t*>(data);
  while (byte_count > 0) {
    int n = (pread(fd, p, byte_count, offset));
    if (n <= 0) return false;
    p += n;
    byte_count -= n;
    offset += n;
...
bool Readlink(const std::string& path, std::string* result) {
  ...
  while (true) {
    int size = readlink(path.c_str(), &buf[0], buf.size());
    // Unrecoverable error?
