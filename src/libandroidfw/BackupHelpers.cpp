...
read_snapshot_file(int fd, KeyedVector<String8,FileState>* snapshot)
{
    ...
    if (header.magic0 != MAGIC0 || header.magic1 != MAGIC1) {
        printf("read_snapshot_file header.magic0=0x%08x magic1=0x%08x", header.magic0, header.magic1);
        return 1;
    }
    ...
    for (int i=0; i<header.fileCount; i++) {
        ...
        if (amt != sizeof(FileState)) {
            printf("read_snapshot_file FileState truncated/error with read at %d bytes\n", bytesRead);
            return 1;
        }
        bytesRead += amt;
        ...
        if (amt != nameBufSize) {
            printf("read_snapshot_file filename truncated/error with read at %d bytes\n", bytesRead);
            return 1;
        }
    }

    if (header.totalSize != bytesRead) {
        printf("read_snapshot_file length mismatch: header.totalSize=%d bytesRead=%d\n",
...
write_snapshot_file(int fd, const KeyedVector<String8,FileRec>& snapshot)
{
    ...
    printf("write_snapshot_file fd=%d\n", fd);
    ...
    if (amt != sizeof(header)) {
        printf("write_snapshot_file error writing header %s", strerror(errno));
        return errno;
    }
    ...
    for (int i=0; i<N; i++) {
        ...
        if (!r.deleted) {
            ...
            if (amt != nameLen) {
                printf("write_snapshot_file error writing filename %s", strerror(errno));
                return 1;
            }
            ...
            if (paddingLen != 0) {
                ...
                if (amt != paddingLen) {
                    printf("write_snapshot_file error writing %d bytes of filename padding %s",
...
{
    printf("write_update_file %s (%s) : mode 0%o\n", realFilename, key.string(), mode);
    ...
    if (sizeof(metadata) != 16) {
        printf("ERROR: metadata block is the wrong size!");
    }
    ...
    if (bytesLeft != 0) {
        ...
        printf("write_update_file size mismatch for %s. expected=%d actual=%d."
        ...
        char const* const* files, char const* const* keys, int fileCount)
{
    ...
    for (int i=0; i<fileCount; i++) {
        ...
        if (err != 0) {
            ...
            if (newSnapshot.indexOfKey(key) >= 0) {
                printf("back_up_files key already in use '%s'", key.string());
                return -1;
            }
    ...
    while (n<N && m<M) {
        ...
        if (cmp < 0) {
            ...
            printf("file removed: %s", p.string());
            write_delete_file(dataStream, p);
            n++;
        } else if (cmp > 0) {
            // file added
            printf("file added: %s crc=0x%08x", g.file.string(), g.s.crc32);
            ...
            printf("%s", q.string());
            printf("  old: modTime=%d,%d mode=%04o size=%-3d crc32=0x%08x",
                    f.modTime_sec, f.modTime_nsec, f.mode, f.size, f.crc32);
            printf("  new: modTime=%d,%d mode=%04o size=%-3d crc32=0x%08x",
            ...
            if (f.modTime_sec != g.s.modTime_sec || f.modTime_nsec != g.s.modTime_nsec
                ...
                if (fd < 0) {
                    printf("Unable to read file for backup: %s", g.file.string());
                } else {
...
static void calc_tar_checksum(char* buf, size_t buf_size) {
    ...
    sprintf_s(buf + 148, buf_size - 148, "%06o", sum); // the trailing space is
                                                      // already in place
}
...
static int write_pax_header_entry(char* buf, size_t buf_size,
	...
	return sprintf_s(buf, buf_size, "%d %s=%s\n", len, key, value);
}
        ...
        BackupDataWriter* writer)
{
    ...
    int err = 0;
	struct _stat64 s;
    if (_stati64(filepath.string(), &s) != 0) {
        err = errno;
        printf("Error %d (%s) from lstat64(%s)", err, strerror(err), filepath.string());
        return err;
    }
    ...
    *outSize = 512 + (needExtended ? 1024 : 0) + 512*((s.st_size + 511)/512);
    ...
    if (fd < 0) {
        err = errno;
        printf("Error %d (%s) from open(%s)", err, strerror(err), filepath.string());
        return err;
    }
    ...
    if (buf == NULL) {
        printf("Out of mem allocating transfer buffer");
        err = ENOMEM;
        goto done;
    }
    ...
    // [ 100 :   8 ] file mode
    sprintf_s(buf + 100, 8, "%06o ", s.st_mode & ~S_IFMT);

    // [ 108 :   8 ] uid -- ignored in Android format; uids are remapped at restore time
    // [ 116 :   8 ] gid -- ignored in Android format
    sprintf_s(buf + 108, 8, "0%lo", (unsigned long)s.st_uid);
    sprintf_s(buf + 116, 8, "0%lo", (unsigned long)s.st_gid);

    // [ 124 :  12 ] file size in bytes
    sprintf_s(buf + 124, 12, "%011llo", (isdir) ? 0LL : s.st_size);

    // [ 136 :  12 ] last mod time as a UTC time_t
    sprintf_s(buf + 136, 12, "%0lo", (unsigned long)s.st_mtime);
    ...
    if (isdir) {
        ...
        printf("Error: unknown file mode 0%o [%s]", s.st_mode, filepath.string());
        goto cleanup;
    }
    buf[156] = type;
    ...
    if (needExtended) {
        ...
        int paxLen = 0;
        sprintf_s(sizeStr, sizeof(sizeStr), "%lld", (long long)s.st_size);
        ...
        sprintf_s(paxHeader, 100, "PaxHeader/%s", leaf.string());
        ...
        sprintf_s(paxHeader + 124, 12, "%011o", (unsigned int)paxLen);
    ...
    if (!isdir) {
        ...
        while (toWrite > 0) {
            ...
            if (nRead < 0) {
                err = errno;
                printf("Unable to read file [%s], err=%d (%s)", filepath.string(),
                        err, strerror(err));
                break;
            } else if (nRead == 0) {
                printf("EOF but expect %lld more bytes in [%s]", (long long) toWrite,
                        filepath.string());
                err = EIO;
                break;
            }
...
RestoreHelperBase::WriteFile(const String8& filename, BackupDataReader* in)
{
    ...
    if (amt != sizeof(metadata)) {
        printf("Could not read metadata for %s -- %ld / %s", filename.string(),
    ...
    if (metadata.version > CURRENT_METADATA_VERSION) {
        if (!m_loggedUnknownMetadata) {
            ...
            printf("Restoring file with unsupported metadata version %d (currently %d)",
    ...
    if (fd == -1) {
        printf("Could not open file %s -- %s", filename.string(), strerror(errno));
        return errno;
    }
    ...
    while ((amt = in->ReadEntityData(buf, RESTORE_BUF_SIZE)) > 0) {
        ...
        if (err != amt) {
            close(fd);
            printf("Error '%s' writing '%s'", strerror(errno), filename.string());
            return errno;
        }
        crc = crc32(crc, (Bytef*)buf, amt);
    }
    ...
    if (err != 0) {
        printf("Error stating file that we just created %s", filename.string());
        return errno;
    }
