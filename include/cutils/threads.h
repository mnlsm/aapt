...
static _inline void  mutex_lock(mutex_t*  lock)
{
    if (!lock->init) {
        lock->init = 1;
...
static _inline void  mutex_unlock(mutex_t*  lock)
{
    LeaveCriticalSection(lock->lock);
}
static _inline int  mutex_init(mutex_t*  lock)
{
    InitializeCriticalSection(lock->lock);
    lock->init = 2;
    return 0;
}
static _inline void  mutex_destroy(mutex_t*  lock)
{
    if (lock->init) {
        lock->init = 0;
