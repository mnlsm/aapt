...
void Asset::unregisterAsset(Asset* asset)
{
    ...
    if (kIsDebug) {
        printf("Destroying Asset in %p #%d\n", asset, gCount);
    }
}
...
#if 1
    length = _lseeki64(fd, 0, SEEK_END);
    if (length < 0) {
        ::close(fd);
        return NULL;
    }
    (void) _lseeki64(fd, 0, SEEK_SET);
#else
    struct stat st;
    if (fstat(fd, &st) < 0) {
        ::close(fd);
    ...
    AccessMode mode)
{
    ...
    fclose(fp);
    if (!scanResult) {
        printf("File '%s' is not in gzip format\n", fileName);
        ::close(fd);
        return NULL;
    }
...
off64_t Asset::handleSeek(off64_t offset, int whence, off64_t curPosn, off64_t maxPosn)
{
    ...
    if (newOffset < 0 || newOffset > maxPosn) {
        printf("seek out of range: want %ld, end=%ld\n",
...
status_t _FileAsset::openChunk(const char* fileName, int fd, off64_t offset, size_t length)
{
     ...
     */
    off64_t fileLength;
    fileLength = _lseeki64(fd, 0, SEEK_END);
    if (fileLength == (off64_t) -1) {
        // probably a bad file descriptor
        printf("failed lseek (errno=%d)\n", errno);
        return UNKNOWN_ERROR;
    }

    if ((off64_t) (offset + length) > fileLength) {
        printf("start (%ld) + len (%ld) > end (%ld)\n",
...
const void* _FileAsset::getBuffer(bool wordAligned)
{
    ...
    if (mLength < kReadVsMapThreshold) {
        ...
        if (buf == NULL) {
            printf("alloc of %ld bytes failed\n", (long) allocLen);
            return NULL;
        }

        printf("Asset %p allocating buffer size %d (smaller than threshold)", this, (int)allocLen);
        if (mLength > 0) {
            long oldPosn = ftell(mFp);
            fseek(mFp, mStart, SEEK_SET);
            if (fread(buf, 1, mLength, mFp) != (size_t) mLength) {
                printf("failed reading %ld bytes\n", (long) mLength);
                delete[] buf;
                return NULL;
            }
        ...
        printf(" getBuffer: loaded into buffer\n");
...
const void* _FileAsset::ensureAlignment(FileMap* map)
{
    ...
    if ((((size_t)data)&0x3) == 0) {
        ...
        // boundary.
        printf("Returning aligned FileAsset %p (%s).", this,
                getAssetSource());
        return data;
    }
    // If not aligned on a word boundary, then we need to copy it into
    // our own buffer.
    printf("Copying FileAsset %p (%s) to buffer size %d to make it aligned.", this,
