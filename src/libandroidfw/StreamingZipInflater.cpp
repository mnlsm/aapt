...
void StreamingZipInflater::initInflateState() {
    printf("Initializing inflate state");
    ...
    if (mDataMap == NULL) {
        ::_lseek(mFd, mInFileStart, SEEK_SET);
 ...
 */
int StreamingZipInflater::read(void* outBuf, size_t count) {
    ...
    while (toRead > 0) {
        ...
        if (toRead > 0) {
            ...
            if ((mInflateState.avail_in == 0) && (mDataMap == NULL)) {
                ...
                if (err < 0) {
                    printf("Unable to access asset data: %d", err);
                    if (!mStreamNeedsInit) {
                        ::inflateEnd(&mInflateState);
                        initInflateState();
                    }
            ...
            */
            int result = Z_OK;
            if (mStreamNeedsInit) {
                printf("Initializing zlib to inflate");
            ...
            if (result < 0) {
                ...
                printf("Error inflating asset: %d", result);
                ::inflateEnd(&mInflateState);
                initInflateState();
                return -1;
            } else {
...
int StreamingZipInflater::readNextChunk() {
    ...
    if (mInNextChunkOffset < mInTotalSize) {
        ...
        if (toRead > 0) {
            int didRead = ::read(mFd, mInBuf, toRead);
            if (kIsDebug) {
                printf("Reading input chunk, size %08zx didread %08zx", toRead, didRead);
            }
            if (didRead < 0) {
				printf("Error reading asset data: %s", strerror(errno));
                return didRead;
            } else {
