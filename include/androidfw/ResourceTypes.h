 ...
 */
#pragma pack(push, 1)
struct __declspec(align(4)) /*alignas(uintptr_t) */ Res_png_9patch
{
...
struct Res_value
{
	...
	bool operator<(const Res_value& other) const {
...
struct ResStringPool_span
{
	...
	bool operator<(const ResStringPool_span& other) const {
		...
		return name.index < other.name.index;
	}
...
const size_t kResTableTypeMinSize =
sizeof(ResTable_type) - sizeof(ResTable_config) + sizeof(decltype(ResTable_config::size)); //sizeof(ResTable_config::size);
