...
public:
    ...
    StreamingZipInflater(int fd, int64_t compDataStart, size_t uncompSize, size_t compSize);
    ...
    int read(void* outBuf, size_t count);

    // seeking backwards requires uncompressing fom the beginning, so is very
    // expensive.  seeking forwards only requires uncompressing from the current
    // position to the destination.
    int64_t seekAbsolute(int64_t absoluteInputPosition);
...
private:
    ...
    int mFd;
    int64_t mInFileStart;         // where the compressed data lives in the file
    ...
    int64_t mOutCurPosition;      // current position in total offset
