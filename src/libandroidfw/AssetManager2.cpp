...
uint32_t AssetManager2::GetResourceId(const std::string& resource_name,
  ...
  StringPiece16 kAttr16 = (const char16_t*)L"attr";
  std::u16string kAttrPrivate16 = (const char16_t*)L"^attr-private";
...
bool Theme::ApplyStyle(uint32_t resid, bool force) {
  ...
  for (auto bag_iter = begin(bag); bag_iter != bag_iter_end; ++bag_iter) {
    ...
    if (type == nullptr) {
      ...
      uint32_t kInitialCapacity = (1024u - sizeof(Type)) / sizeof(Entry);
      const uint32_t initial_capacity = max(entry_idx, kInitialCapacity);
      type.reset(
