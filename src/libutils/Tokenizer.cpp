...
status_t Tokenizer::open(const String8& filename, Tokenizer** outTokenizer) {
    *outTokenizer = NULL;
    ...
    int result = NO_ERROR;
	FILE* fd = NULL;
	::fopen_s(&fd, filename.string(), O_RDONLY);
    if (fd == 0) {
        result = -errno;
    } else {
        struct stat stat;
		if (::stat(filename.string(), &stat)) {
            result = -errno;
        } else {
			...
			if (TRUE) {
                ...
                ownBuffer = true;
				ssize_t nrd = fread(buffer, 1, length, fd);
                if (nrd < 0) {
                    result = -errno;
                    delete[] buffer;
                    buffer = NULL;
            ...
            if (!result) {
                *outTokenizer = new Tokenizer(filename, NULL, buffer, ownBuffer, length);
            }
        }
        fclose(fd);
    }
