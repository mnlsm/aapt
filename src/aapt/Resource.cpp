...
static uint16_t getMaxSdkVersion(const sp<ResourceTable::ConfigList>& configList) {
  ...
  for (size_t i = 0; i < entries.size(); i++) {
    maxSdkVersion = max(maxSdkVersion, entries.keyAt(i).sdkVersion);
  }
                        ...
                        for (size_t k = 0; k < densityVaryingResourceCount; k++) {
                            ...
                            if (block < 0) {
                                ...
                                if (bundle->getVerbose()) {
                                    ...
                                    const size_t defCount = min(size_t(5), defs.size());
    ...
    for (i=0; i<N; i++) {
        ...
        for (a=0; a<NA; a++) {
            ...
            if (pos >= 0) {
				...
				if (comment.contains(String16("@removed"))) {
                    continue;
                }
                if (comment.size() > 0) {
...
struct NamespaceAttributePair {
	...
	bool operator<(const NamespaceAttributePair& other) const {
		int cmp1 = strcmp(ns, other.ns);
		int cmp2 = strcmp(attr, other.attr);
		if (cmp1 == 0) {
			return (cmp2 == -1);
		}
		return (cmp1 == -1);
