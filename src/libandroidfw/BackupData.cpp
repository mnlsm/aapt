     ...
     m_entityCount(0)
{
    ...
    if (kIsDebug) printf("BackupDataWriter(%d) @ %ld", fd, (long)m_pos);
}
...
BackupDataWriter::write_padding_for(int n)
{
    ...
    if (paddingSize > 0) {
        ...
        if (kIsDebug) printf("writing %zd padding bytes for %d", paddingSize, n);
...
BackupDataWriter::WriteEntityHeader(const String8& key, size_t dataSize)
{
    ...
    if (kIsDebug) {
        printf("Writing header: prefix='%s' key='%s' dataSize=%zu", m_keyPrefix.string(),
                key.string(), dataSize);
    }
    ...
    if (kIsDebug) printf("writing entity header, %zu bytes", sizeof(entity_header_v1));
    ...
    if (kIsDebug) printf("writing entity header key, %zd bytes", keyLen+1);
...
{
    if (kIsDebug) printf("Writing data: size=%lu", (unsigned long) size);

    if (m_status != NO_ERROR) {
        if (kIsDebug) {
            printf("Not writing data - stream in error state %d (%s)", m_status, strerror(m_status));
        }
        return m_status;
    }
    ...
    if (amt != (ssize_t)size) {
        m_status = errno;
        if (kIsDebug) printf("write returned error %d (%s)", m_status, strerror(m_status));
        return m_status;
    }
    m_pos += amt;
    return NO_ERROR;
     ...
     m_entityCount(0)
{
    ...
    m_pos = (ssize_t) lseek(fd, 0, SEEK_CUR);
    if (kIsDebug) printf("BackupDataReader(%d) @ %ld", fd, (long)m_pos);
}
    ...
    do { \
        if ((actual) != (expected)) { \
            if ((actual) == 0) { \
                ...
                m_status = errno; \
                printf("CHECK_SIZE(a=%ld e=%ld) failed at line %d m_status='%s'", \
...
#define SKIP_PADDING() \
    do { \
        ...
        if (err != NO_ERROR) { \
            printf("SKIP_PADDING FAILED at line %d", __LINE__); \
            m_status = err; \
            return err; \
        } \
    } while(0)
    ...
    switch (m_header.type)
    {
        case BACKUP_HEADER_ENTITY_V1:
        {
            ...
            if (m_header.entity.keyLen <= 0) {
                printf("Entity header at %d has keyLen<=0: 0x%08x\n", (int)m_pos,
        ...
        default:
            printf("Chunk header at %d has invalid type: 0x%08x",
...
BackupDataReader::ReadEntityData(void* data, size_t size)
{
    ...
    if (kIsDebug) {
        printf("ReadEntityData size=%zu m_pos=0x%zx m_dataEndPos=0x%zx remaining=%d\n",
