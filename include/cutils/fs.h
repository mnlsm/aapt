 ...
 */
extern int fs_prepare_dir(const char* path, unsigned int mode, int uid, int gid);

/*
 * Ensure that directory exists with given mode and owners.  If it exists
 * with different owners, they are not fixed and -1 is returned.
 */
extern int fs_prepare_dir_strict(const char* path, unsigned int mode, int uid, int gid);

/*
 * Ensure that file exists with given mode and owners.  If it exists
 * with different owners, they are not fixed and -1 is returned.
 */
extern int fs_prepare_file_strict(const char* path, unsigned int mode, int uid, int gid);
 ...
 */
extern int fs_mkdirs(const char* path, unsigned int mode);
