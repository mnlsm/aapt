...
public:
     ...
     * lseek/fseek.  Returns the new position on success, or (int64_t) -1
     * on failure.
     */
     ...
     */
    virtual int64_t getRemainingLength(void) const = 0;
     ...
     * asset is compressed).
     */
    virtual int openFileDescriptor(int64_t* outStart, int64_t* outLength) const = 0;
...
protected:
    ...
    int64_t handleSeek(int64_t offset, int whence, int64_t curPosn, int64_t maxPosn);
...
private:
     ...
     */
    static Asset* createFromFileSegment(int fd, int64_t offset, size_t length,
        AccessMode mode);
     ...
     * use.
     */
    static Asset* createFromCompressedData(int fd, int64_t offset,
...
public:
     ...
     */
    status_t openChunk(const char* fileName, int fd, int64_t offset, size_t length);
    ...
    virtual int64_t getLength(void) const { return mLength; }
    virtual int64_t getRemainingLength(void) const { return mLength-mOffset; }
    virtual int openFileDescriptor(int64_t* outStart, int64_t* outLength) const;
    virtual bool isAllocated(void) const { return mBuf != NULL; }

private:
    int64_t     mStart;         // absolute file offset of start of chunk
    int64_t     mLength;        // length of the chunk
    int64_t     mOffset;        // current local offset, 0 == mStart
...
public:
     ...
     * On success, the object takes ownership of "fd".
     */
    status_t openChunk(int fd, int64_t offset, int compressionMethod,
    ...
    virtual void close(void);
    virtual const void* getBuffer(bool wordAligned);
    virtual int64_t getLength(void) const { return mUncompressedLen; }
    virtual int64_t getRemainingLength(void) const { return mUncompressedLen-mOffset; }
    virtual int openFileDescriptor(int64_t* /* outStart */, int64_t* /* outLength */) const { return -1; }
    virtual bool isAllocated(void) const { return mBuf != NULL; }

private:
    int64_t     mStart;         // offset to start of compressed data
    int64_t     mCompressedLen; // length of the compressed data
    int64_t     mUncompressedLen; // length of the uncompressed data
    int64_t     mOffset;        // current offset, 0 == start of uncomp data
