...
static bool assertIdmapHeader(const void* idmap, size_t size) {
    if (reinterpret_cast<uintptr_t>(idmap) & 0x03) {
        printf("idmap: header is not word aligned");
        return false;
    }

    if (size < ResTable::IDMAP_HEADER_SIZE_BYTES) {
        printf("idmap: header too small (%d bytes)", (uint32_t) size);
        return false;
    }

    const uint32_t magic = htodl(*reinterpret_cast<const uint32_t*>(idmap));
    if (magic != IDMAP_MAGIC) {
        printf("idmap: no magic found in header (is 0x%08x, expected 0x%08x)",
             magic, IDMAP_MAGIC);
        return false;
    }
    ...
    if (version != ResTable::IDMAP_CURRENT_VERSION) {
        ...
        printf("idmap: version mismatch in header (is 0x%08x, expected 0x%08x)",
...
public:
    ...
    status_t setTo(const void* entryHeader, size_t size) {
        if (reinterpret_cast<uintptr_t>(entryHeader) & 0x03) {
            printf("idmap: entry header is not word aligned");
            return UNKNOWN_ERROR;
        }

        if (size < sizeof(uint16_t) * 4) {
            printf("idmap: entry header is too small (%u bytes)", (uint32_t) size);
            return UNKNOWN_ERROR;
        }
        ...
        if (targetTypeId == 0 || overlayTypeId == 0 || targetTypeId > 255 || overlayTypeId > 255) {
            printf("idmap: invalid type map (%u -> %u)", targetTypeId, overlayTypeId);
            return UNKNOWN_ERROR;
        }

        uint16_t entryCount = dtohs(header[2]);
        if (size < sizeof(uint32_t) * (entryCount + 2)) {
            printf("idmap: too small (%u bytes) for the number of entries (%u)",
...
status_t parseIdmap(const void* idmap, size_t size, uint8_t* outPackageId, KeyedVector<uint8_t, IdmapEntries>* outMap) {
    ...
    if (size < sizeof(uint16_t) * 2) {
        printf("idmap: too small to contain any mapping");
        return UNKNOWN_ERROR;
    }
    ...
    if (targetPackageId == 0 || targetPackageId > 255) {
        printf("idmap: target package ID is invalid (%02x)", targetPackageId);
        return UNKNOWN_ERROR;
    }

    uint16_t mapCount = dtohs(*(data++));
    if (mapCount == 0) {
        printf("idmap: no mappings");
        return UNKNOWN_ERROR;
    }

    if (mapCount > 255) {
        printf("idmap: too many mappings. Only 255 are possible but %u are present", (uint32_t) mapCount);
    }
...
status_t ResStringPool::setTo(const void* data, size_t size, bool copyData)
{
    ...
    if (mHeader->styleCount > 0) {
        ...
        if (mEntryStyles < mEntries) {
            printf("Bad string block: integer overflow finding styles\n");
            return (mError=BAD_TYPE);
        }

        if (((const uint8_t*)mEntryStyles-(const uint8_t*)mHeader) > (int)size) {
            printf("Bad string block: entry of %d styles extends past data size %d\n",
        ...
        if (mHeader->stylesStart >= mHeader->header.size) {
            printf("Bad string block: style pool starts %d, after total size %d\n",
                    (int)mHeader->stylesStart, (int)mHeader->header.size);
            return (mError=BAD_TYPE);
        }
        mStylePoolSize =
        ...
        if (memcmp(&mStyles[mStylePoolSize-(sizeof(endSpan)/sizeof(uint32_t))],
            ...
            printf("Bad string block: last style is not 0xFFFFFFFF-terminated\n");
            return (mError=BAD_TYPE);
        }
    } else {
        mEntryStyles = NULL;
                ...
                *u16len = decodeLength(&str);
                if ((uint32_t)(str+*u16len-strings) < mStringPoolSize) {
                    ...
                    if (str[*u16len] != 0x0000) {
                        printf("Bad string block: string #%d is not null-terminated",
                              (int)idx);
                        return NULL;
                    }
                ...
                if ((uint32_t)(u8str+u8len-strings) < mStringPoolSize) {
                    ...
                    if (mCache == NULL) {
                        ...
                        if (kDebugStringPoolNoisy) {
                            printf("CREATING STRING CACHE OF %zu bytes",
                        ...
                        if (mCache == NULL) {
                            printf("No memory trying to allocate decode cache table of %d bytes\n",
                    ...
                    if (actualLen < 0 || (size_t)actualLen != *u16len) {
                        printf("Bad string block: string #%lld decoded length is not correct "
                                "%lld vs %llu\n",
                    ...
                    if (!u16str) {
                        printf("No memory when trying to allocate decode cache for string #%d\n",
                                (int)idx);
                        return NULL;
                    }

                    if (kDebugStringPoolNoisy) {
                        printf("Caching UTF8 string: %s", u8str);
                    }
...
const ResStringPool_span* ResStringPool::styleAt(size_t idx) const
{
    if (mError == NO_ERROR && idx < mHeader->styleCount) {
        ...
        if (off < mStylePoolSize) {
            ...
            printf("Bad string block: style #%d entry is at %d, past end at %d\n",
...
ssize_t ResStringPool::indexOfString(const char16_t* str, size_t strLen) const
{
    ...
    if ((mHeader->flags&ResStringPool_header::UTF8_FLAG) != 0) {
        if (kDebugStringPoolNoisy) {
            printf("indexOfString UTF-8: %s", String8(str, strLen).string());
        }
        ...
        if (mHeader->flags&ResStringPool_header::SORTED_FLAG) {
            ...
            while (l <= h) {
                ...
                if (kDebugStringPoolNoisy) {
                    printf("Looking at %s, cmp=%d, l/mid/h=%d/%d/%d\n",
        ...
        if (kDebugStringPoolNoisy) {
            printf("indexOfString UTF-16: %s", String8(str, strLen).string());
        }
                                       ...
                                       const char16_t* attr, size_t attrLen) const
{
    if (mEventCode == START_TAG) {
        ...
        if (mTree.mStrings.isUTF8()) {
            ...
            if (kDebugStringPoolNoisy) {
                printf("indexOfAttribute UTF8 %s (%zu) / %s (%zu)", ns8.string(), nsLen,
            ...
            for (size_t i=0; i<N; i++) {
                ...
                if (kDebugStringPoolNoisy) {
                    printf("  curNs=%s (%zu), curAttr=%s (%zu)", curNs, curNsLen, curAttr, curAttrLen);
                }
            ...
            if (kDebugStringPoolNoisy) {
                printf("indexOfAttribute UTF16 %s (%zu) / %s (%zu)",
            ...
            for (size_t i=0; i<N; i++) {
                ...
                if (kDebugStringPoolNoisy) {
                    printf("  curNs=%s (%zu), curAttr=%s (%zu)",
...
ResXMLParser::event_code_t ResXMLParser::nextNode()
{
    ...
    do {
        ...
        if (kDebugXMLNoisy) {
            printf("Next node: prev=%p, next=%p\n", mCurNode, next);
        }
        ...
        switch ((mEventCode=eventCode)) {
            ...
            case RES_XML_CDATA_TYPE:
                ...
                break;
            default:
                printf("Unknown XML block: header type %d in node at %d\n",
...
{
    if (kDebugResXMLTree) {
        printf("Creating ResXMLTree %p #%d\n", this, android_atomic_inc(&gCount)+1);
    }
    restart();
}
...
{
    if (kDebugResXMLTree) {
        printf("Destroying ResXMLTree in %p #%d\n", this, android_atomic_dec(&gCount)-1);
    }
    uninit();
}
...
status_t ResXMLTree::validateNode(const ResXMLTree_node* node) const
{
    ...
    if (headerSize >= (isStart ? sizeof(ResXMLTree_attrNode) : sizeof(ResXMLTree_node))) {
        if (size >= headerSize) {
            ...
            printf("Bad XML block: node at 0x%x extends beyond data end 0x%x\n",
                    (int)(((const uint8_t*)node)-((const uint8_t*)mHeader)), (int)mSize);
            return BAD_TYPE;
        }
        printf("Bad XML block: node at 0x%x header size 0x%x smaller than total size 0x%x\n",
                (int)(((const uint8_t*)node)-((const uint8_t*)mHeader)),
                (int)headerSize, (int)size);
        return BAD_TYPE;
    }
    printf("Bad XML block: node at 0x%x header size 0x%x too small\n",
            (int)(((const uint8_t*)node)-((const uint8_t*)mHeader)),
            (int)headerSize);
    return BAD_TYPE;
#endif
    ...
    if (requested) {
        ...
        if (screenSizeDp || o.screenSizeDp) {
            ...
            if (kDebugTableSuperNoisy) {
                printf("Comparing this %dx%d to other %dx%d in %dx%d: myDelta=%d otherDelta=%d",
...
bool ResTable_config::match(const ResTable_config& settings) const {
    ...
    if (screenSizeDp != 0) {
        if (screenWidthDp != 0 && screenWidthDp > settings.screenWidthDp) {
            if (kDebugTableSuperNoisy) {
                printf("Filtering out width %d in requested %d", screenWidthDp,
        ...
        if (screenHeightDp != 0 && screenHeightDp > settings.screenHeightDp) {
            if (kDebugTableSuperNoisy) {
                printf("Filtering out height %d in requested %d", screenHeightDp,
    ...
    if (input != 0) {
        ...
        if (keysHidden != 0 && keysHidden != setKeysHidden) {
            ...
            if (kDebugTableSuperNoisy) {
                printf("Matching keysHidden: have=%d, config=%d\n", keysHidden, setKeysHidden);
            }
...
status_t ResTable::Theme::applyStyle(uint32_t resID, bool force)
{
    ...
    if (kDebugTableNoisy) {
        printf("Applying style 0x%08x to theme %p, count=%zu", resID, this, N);
    }
    if (N < 0) {
        mTable.unlock();
        return N;
    ...
    while (bag < end) {
        ...
        if (curPackage != p) {
            ...
            if (pidx < 0) {
                printf("Style contains key with bad package: 0x%08x\n", attrRes);
                bag++;
                continue;
            }
            curPackage = p;
        ...
        if (curType != t) {
            if (t > Res_MAXTYPE) {
                printf("Style contains key with bad type: 0x%08x\n", attrRes);
                bag++;
                continue;
            }
            curType = t;
        ...
        if (e >= numEntries) {
            printf("Style contains key with bad entry: 0x%08x\n", attrRes);
            bag++;
            continue;
        }
        theme_entry* curEntry = curEntries + e;
        if (kDebugTableNoisy) {
            printf("Attr 0x%08x: type=0x%x, data=0x%08x; curType=0x%x",
    ...
    if (kDebugTableTheme) {
        printf("Applying style 0x%08x (force=%d)  theme %p...\n", resID, force, this);
        dumpToLog();
    }
...
{
    if (kDebugTableTheme) {
        printf("Setting theme %p from theme %p...\n", this, &other);
        dumpToLog();
        other.dumpToLog();
    }
        ...
        uint32_t* outTypeSpecFlags) const
{
    ...
    do {
        ...
        if (kDebugTableTheme) {
            printf("Looking up attr 0x%08x in theme %p", resID, this);
        }

        if (p >= 0) {
            const package_info* const pi = mPackages[p];
            if (kDebugTableTheme) {
                printf("Found package: %p", pi);
            }
            if (pi != NULL) {
                if (kDebugTableTheme) {
                    printf("Desired type index is %u in avail %zu", t, Res_MAXTYPE + 1);
                }
                if (t <= Res_MAXTYPE) {
                    const type_info& ti = pi->types[t];
                    if (kDebugTableTheme) {
                        printf("Desired entry index is %u in avail %zu", e, ti.numEntries);
                    }
                    if (e < ti.numEntries) {
                        ...
                        if (kDebugTableTheme) {
                            printf("Theme value: type=0x%x, data=0x%08x",
                        ...
                        if (type == Res_value::TYPE_ATTRIBUTE) {
                            ...
                            printf("Too many attribute references, stopped at: 0x%08x\n", resID);
                            return BAD_INDEX;
        ...
        uint32_t* inoutTypeSpecFlags, ResTable_config* inoutConfig) const
{
    ...
    if (inOutValue->dataType == Res_value::TYPE_ATTRIBUTE) {
        ...
        if (kDebugTableTheme) {
            printf("Resolving attr reference: blockIndex=%d, type=0x%x, data=0x%x\n",
    ...
    for (size_t i=0; i<Res_MAXPACKAGE; i++) {
        ...
        for (size_t j = 0; j <= Res_MAXTYPE; j++) {
            ...
            for (size_t k = 0; k < ti.numEntries; k++) {
                ...
                printf("      0x%08x: t=0x%x, d=0x%08x (block=%d)\n",
...
{
    if (kDebugTableSuperNoisy) {
        printf("Destroying ResTable in %p\n", this);
    }
    uninit();
}
...
status_t ResTable::add(Asset* asset, const int32_t cookie, bool copyData) {
    ...
    if (data == NULL) {
        printf("Unable to get buffer of resource asset file");
        return UNKNOWN_ERROR;
    }
...
status_t ResTable::add(
    ...
    if (idmapAsset != NULL) {
        ...
        if (idmapData == NULL) {
            printf("Unable to get buffer of idmap asset file");
            return UNKNOWN_ERROR;
        }
        ...
        bool appAsLib, const int32_t cookie, bool copyData, bool isSystemAsset)
{
    ...
    if (dataSize < sizeof(ResTable_header)) {
        printf("Invalid data. Size(%d) is smaller than a ResTable_header(%d).",
    ...
    if (kDebugLoadTableNoisy) {
        printf("Adding resources to ResTable: data=%p, size=%zu, cookie=%d, copy=%d "
    ...
    if (kDebugLoadTableSuperNoisy) {
        printf("Got size %zu, again size 0x%x, raw size 0x%x\n", header->size,
                dtohl(header->header->header.size), header->header->header.size);
    }
    if (kDebugLoadTableNoisy) {
        printf("Loading ResTable @%p:\n", header->header);
    }
    if (dtohs(header->header->header.headerSize) > header->size
            || header->size > dataSize) {
        printf("Bad resource table: header size 0x%x or total size 0x%x is larger than data size 0x%x\n",
             (int)dtohs(header->header->header.headerSize),
             (int)header->size, (int)dataSize);
        return (mError=BAD_TYPE);
    }
    if (((dtohs(header->header->header.headerSize)|header->size)&0x3) != 0) {
        printf("Bad resource table: header size 0x%x or total size 0x%x is not on an integer boundary\n",
             (int)dtohs(header->header->header.headerSize),
             (int)header->size);
        return (mError=BAD_TYPE);
    }
    ...
    while (((const uint8_t*)chunk) <= (header->dataEnd-sizeof(ResChunk_header)) &&
        ...
        if (kDebugTableNoisy) {
            printf("Chunk: type=0x%x, headerSize=0x%x, size=0x%x, pos=%p\n",
        ...
        if (ctype == RES_STRING_POOL_TYPE) {
            if (header->values.getError() != NO_ERROR) {
                ...
                printf("Multiple string chunks found in resource table.");
            }
        } else if (ctype == RES_TABLE_PACKAGE_TYPE) {
            if (curPackage >= dtohl(header->header->packageCount)) {
                printf("More package chunks were found than the %d declared in the header.",
            ...
            printf("Unknown chunk type 0x%x in table at %p.\n",
                 ctype,
                 (void*)(((const uint8_t*)chunk) - ((const uint8_t*)header->header)));
        }
        chunk = (const ResChunk_header*)
    ...
    if (curPackage < dtohl(header->header->packageCount)) {
        printf("Fewer package chunks (%d) were found than the %d declared in the header.",
             (int)curPackage, dtohl(header->header->packageCount));
        return (mError=BAD_TYPE);
    }
    mError = header->values.getError();
    if (mError != NO_ERROR) {
        printf("No string values found in resource table!");
    }

    if (kDebugTableNoisy) {
        printf("Returning from add with mError=%d\n", mError);
    }
    return mError;
}
...
bool ResTable::getResourceName(uint32_t resID, bool allowUtf8, resource_name* outName) const
{
    ...
    if (p < 0) {
        if (Res_GETPACKAGE(resID)+1 == 0) {
            printf("No package identifier when getting name for resource number 0x%08x", resID);
        } else {
#ifndef STATIC_ANDROIDFW_FOR_TOOLS
            printf("No known package when getting name for resource number 0x%08x", resID);
#endif
        }
        return false;
    }
    if (t < 0) {
        printf("No type identifier when getting name for resource number 0x%08x", resID);
        return false;
    }

    const PackageGroup* const grp = mPackageGroups[p];
    if (grp == NULL) {
        printf("Bad identifier when getting name for resource number 0x%08x", resID);
        return false;
    }
        ...
        uint32_t* outSpecFlags, ResTable_config* outConfig) const
{
    ...
    if (p < 0) {
        if (Res_GETPACKAGE(resID)+1 == 0) {
            printf("No package identifier when getting value for resource number 0x%08x", resID);
        } else {
            printf("No known package when getting value for resource number 0x%08x", resID);
        }
        return BAD_INDEX;
    }
    if (t < 0) {
        printf("No type identifier when getting value for resource number 0x%08x", resID);
        return BAD_INDEX;
    }

    const PackageGroup* const grp = mPackageGroups[p];
    if (grp == NULL) {
        printf("Bad identifier when getting value for resource number 0x%08x", resID);
        return BAD_INDEX;
    }
...
#ifndef STATIC_ANDROIDFW_FOR_TOOLS
        printf("Failure getting entry for 0x%08x (t=%d e=%d) (error %d)\n",
                resID, t, e, err);
#endif
        return err;
    }
    ...
    if ((dtohs(entry.entry->flags) & ResTable_entry::FLAG_COMPLEX) != 0) {
        if (!mayBeBag) {
            printf("Requesting resource 0x%08x failed because it is complex\n", resID);
        }
        return BAD_VALUE;
    }
    ...
    if (grp->dynamicRefTable.lookupResourceValue(outValue) != NO_ERROR) {
        printf("Failed to resolve referenced package: 0x%08x", outValue->data);
        return BAD_VALUE;
    }
        ...
        ResTable_config* outConfig) const
{
    ...
    while (blockIndex >= 0 && value->dataType == Res_value::TYPE_REFERENCE
        ...
        if (kDebugTableTheme) {
            printf("Resolving reference 0x%x: newIndex=%d, type=0x%x, data=0x%x\n",
        ...
        uint32_t* outTypeSpecFlags) const
{
    ...
    if (p < 0) {
        printf("Invalid package identifier when getting bag for resource number 0x%08x", resID);
        return BAD_INDEX;
    }
    if (t < 0) {
        printf("No type identifier when getting bag for resource number 0x%08x", resID);
        return BAD_INDEX;
    }
    ...
    if (grp == NULL) {
        printf("Bad identifier when getting bag for resource number 0x%08x", resID);
        return BAD_INDEX;
    }

    const TypeList& typeConfigs = grp->types[t];
    if (typeConfigs.isEmpty()) {
        printf("Type identifier 0x%x does not exist.", t+1);
        return BAD_INDEX;
    }

    const size_t NENTRY = typeConfigs[0]->entryCount;
    if (e >= (int)NENTRY) {
        printf("Entry identifier 0x%x is larger than entry count 0x%x",
        ...
        if (set) {
            if (set != (bag_set*)0xFFFFFFFF) {
                ...
                *outBag = (bag_entry*)(set+1);
                if (kDebugTableSuperNoisy) {
                    printf("Found existing bag for: 0x%x\n", resID);
                }
                return set->numAttrs;
            }
            printf("Attempt to retrieve bag 0x%08x which is invalid or in a cycle.",
                 resID);
            return BAD_INDEX;
        }
    }
    ...
    if (kDebugTableNoisy) {
        printf("Found map: size=%x parent=%x count=%d\n", entrySize, parent, count);
    ...
    if (parent) {
        ...
        if (err != NO_ERROR) {
            printf("Failed resolving bag parent id 0x%08x", parent);
            return UNKNOWN_ERROR;
        }
        ...
        if (NP > 0) {
            ...
            set->numAttrs = NP;
            if (kDebugTableNoisy) {
                printf("Initialized new bag with %zd inherited attributes.\n", NP);
            }
        } else {
            if (kDebugTableNoisy) {
                printf("Initialized new bag with no inherited attributes.\n");
            }
            set->numAttrs = 0;
        }
        set->availAttrs = NT;
    ...
    uint32_t pos = 0;
    if (kDebugTableNoisy) {
        printf("Starting with set %p, entries=%p, avail=%zu\n", set, entries, set->availAttrs);
    }
    while (pos < count) {
        if (kDebugTableNoisy) {
            printf("Now at %p\n", (void*)curOff);
        ...
        if (curOff > (dtohl(entry.type->header.size)-sizeof(ResTable_map))) {
            printf("ResTable_map at %d is beyond type chunk data %d",
        ...
        if (!Res_INTERNALID(newName)) {
            ...
            if (grp->dynamicRefTable.lookupResourceId(&newName) != NO_ERROR) {
                printf("Failed resolving ResTable_map name at %d with ident 0x%08x",
        ...
        while ((isInside=(curEntry < set->numAttrs))
            ...
            if (kDebugTableNoisy) {
                printf("#%zu: Keeping existing attribute: 0x%08x\n",
        ...
        if ((!isInside) || oldName != newName) {
            ...
            if (set->numAttrs >= set->availAttrs) {
                ...
                if (kDebugTableNoisy) {
                    printf("Reallocated set %p, entries=%p, avail=%zu\n",
                            set, entries, set->availAttrs);
                }
            }
            if (isInside) {
            ...
            if (kDebugTableNoisy) {
                printf("#%zu: Inserting new attribute: 0x%08x\n", curEntry, newName);
            }
        } else {
            if (kDebugTableNoisy) {
                printf("#%zu: Replacing existing attribute: 0x%08x\n", curEntry, oldName);
            }
        }
        ...
        if (err != NO_ERROR) {
            printf("Reference item(0x%08x) in bag could not be resolved.", cur->map.value.data);
            return UNKNOWN_ERROR;
        }

        if (kDebugTableNoisy) {
            printf("Setting entry #%zu %p: block=%zd, name=0x%08d, type=%d, data=0x%08x\n",
...
void ResTable::setParameters(const ResTable_config* params)
{
    ...
    for (size_t p = 0; p < mPackageGroups.size(); p++) {
        ...
        if (kDebugTableNoisy) {
            printf("CLEARING BAGS FOR GROUP %zu!", p);
        }
        ...
        for (size_t t = 0; t < packageGroup->types.size(); t++) {
            ...
            for (size_t ts = 0; ts < typeList.size(); ts++) {
                ...
                if (kDebugTableNoisy) {
                    printf("Updating pkg=%zu type=%zu with %zu filtered configs",
        ...
        if (nameLen > 7) {
            if (name[1] == 'i' && name[2] == 'n'
                ...
                if (Res_CHECKID(index)) {
                    printf("Array resource index: %d is too large.",
                         index);
                    return 0;
                }
                if (outTypeSpecFlags) {
                ...
                if (dtohl(entry->key.index) == (size_t) ei) {
                    ...
                    if (outTypeSpecFlags) {
                        ...
                        if (getEntry(group, typeIndex, iter.index(), NULL, &result) != NO_ERROR) {
                            printf("Failed to find spec flags for 0x%08x", resId);
                            return 0;
                        }
...
bool ResTable::getResourceFlags(uint32_t resID, uint32_t* outFlags) const {
    ...
    if (p < 0) {
        if (Res_GETPACKAGE(resID)+1 == 0) {
            printf("No package identifier when getting flags for resource number 0x%08x", resID);
        } else {
            printf("No known package when getting flags for resource number 0x%08x", resID);
        }
        return false;
    }
    if (t < 0) {
        printf("No type identifier when getting flags for resource number 0x%08x", resID);
        return false;
    }

    const PackageGroup* const grp = mPackageGroups[p];
    if (grp == NULL) {
        printf("Bad identifier when getting flags for resource number 0x%08x", resID);
        return false;
    }
        ...
        Entry* outEntry) const
{
    ...
    if (typeList.isEmpty()) {
        printf("Skipping entry type index 0x%02x because type is NULL!\n", typeIndex);
        return BAD_TYPE;
    }
    ...
    for (size_t i = 0; i < typeCount; i++) {
        ...
        if (static_cast<size_t>(realEntryIndex) >= typeSpec->entryCount) {
            printf("For resource 0x%08x, entry index(%d) is beyond type entryCount(%d)",
    ...
    if (bestOffset > (dtohl(bestType->header.size)-sizeof(ResTable_entry))) {
        printf("ResTable_entry at 0x%x is beyond type chunk data 0x%x",
                bestOffset, dtohl(bestType->header.size));
        return BAD_TYPE;
    }
    if ((bestOffset & 0x3) != 0) {
        printf("ResTable_entry at 0x%x is not on an integer boundary", bestOffset);
        return BAD_TYPE;
    }
    ...
    if (dtohs(entry->size) < sizeof(*entry)) {
        printf("ResTable_entry size 0x%x is too small", dtohs(entry->size));
        return BAD_TYPE;
    }
                                ...
                                const Header* const header, bool appAsLib, bool isSystemAsset)
{
    ...
    if (dtohl(pkg->typeStrings) >= pkgSize) {
        printf("ResTable_package type strings at 0x%x are past chunk size 0x%x.",
             dtohl(pkg->typeStrings), pkgSize);
        return (mError=BAD_TYPE);
    }
    if ((dtohl(pkg->typeStrings)&0x3) != 0) {
        printf("ResTable_package type strings at 0x%x is not on an integer boundary.",
             dtohl(pkg->typeStrings));
        return (mError=BAD_TYPE);
    }
    if (dtohl(pkg->keyStrings) >= pkgSize) {
        printf("ResTable_package key strings at 0x%x are past chunk size 0x%x.",
             dtohl(pkg->keyStrings), pkgSize);
        return (mError=BAD_TYPE);
    }
    if ((dtohl(pkg->keyStrings)&0x3) != 0) {
        printf("ResTable_package key strings at 0x%x is not on an integer boundary.",
...
//        LOG_ALWAYS_FATAL("Package id out of range");
    ...
    while (((const uint8_t*)chunk) <= (endPos-sizeof(ResChunk_header)) &&
        ...
        if (kDebugTableNoisy) {
            printf("PackageChunk: type=0x%x, headerSize=0x%x, size=0x%x, pos=%p\n",
                    dtohs(chunk->type), dtohs(chunk->headerSize), dtohl(chunk->size),
                    (void*)(((const uint8_t*)chunk) - ((const uint8_t*)header->header)));
        }
        const size_t csize = dtohl(chunk->size);
        ...
        if (ctype == RES_TABLE_TYPE_SPEC_TYPE) {
            ...
            if ((dtohl(typeSpec->entryCount) > (INT32_MAX/sizeof(uint32_t))
                    ...
                    > typeSpecSize)) {
                printf("ResTable_typeSpec entry index to %p extends beyond chunk end %p.",
            ...
            if (typeSpec->id == 0) {
                printf("ResTable_type has an id of 0.");
                return (mError=BAD_TYPE);
            }
            ...
            if (newEntryCount > 0) {
                ...
                if (addToType) {
                    ...
                    if (!typeList.isEmpty()) {
                        ...
                        if (existingType->entryCount != newEntryCount && idmapIndex < 0) {
                            printf("ResTable_typeSpec entry count inconsistent: "
                ...
                printf("Skipping empty ResTable_typeSpec for type %d", typeSpec->id);
            }
            ...
            if (dtohs(type->header.headerSize)+(sizeof(uint32_t)*newEntryCount) > typeSize) {
                printf("ResTable_type entry index to %p extends beyond chunk end 0x%x.",
            ...
            if (newEntryCount != 0
                ...
                printf("ResTable_type entriesStart at 0x%x extends beyond chunk end 0x%x.",
            ...
            if (newEntryCount > 0) {
                ...
                if (addToType) {
                    TypeList& typeList = group->types.editItemAt(typeIndex);
                    if (typeList.isEmpty()) {
                        printf("No TypeSpec for type %d", type->id);
                        return (mError=BAD_TYPE);
                    }
                    ...
                    if (kDebugTableGetEntry) {
                        ...
                        printf("Adding config to type %d: %s\n", type->id,
                                thisConfig.toString().string());
                    }
                }
            } else {
                printf("Skipping empty ResTable_type for type %d", type->id);
            }
            ...
            if (group->dynamicRefTable.entries().size() == 0) {
                ...
                printf("Found multiple library tables, ignoring...");
            }
        } else {
...
status_t DynamicRefTable::load(const ResTable_lib_header* const header)
{
    ...
    if (sizeOfEntries > expectedSize) {
        printf("ResTable_lib_header size %u is too small to fit %u entries (x %u).",
    ...
    for (uint32_t entryIndex = 0; entryIndex < entryCount; entryIndex++) {
        ...
        if (kDebugLibNoisy) {
            printf("Found lib entry %s with id %d\n", String8(tmpName).string(),
                    dtohl(entry->packageId));
        }
        if (packageId >= 256) {
            printf("Bad package id 0x%08x", packageId);
            return UNKNOWN_ERROR;
        }
...
status_t DynamicRefTable::lookupResourceId(uint32_t* resId) const {
    ...
    if (translatedId == 0) {
        printf("DynamicRefTable(0x%02x): No mapping for build-time package ID 0x%02x.",
                (uint8_t)mAssignedPackageId, (uint8_t)packageId);
        for (size_t i = 0; i < 256; i++) {
            if (mLookupTable[i] != 0) {
                printf("e[0x%02x] -> 0x%02x", (uint8_t)i, mLookupTable[i]);
            }
        }
        return UNKNOWN_ERROR;
    }
        ...
        void** outData, size_t* outSize) const
{
    ...
    if (mPackageGroups.size() == 0) {
        printf("idmap: target package has no package groups, cannot create idmap\n");
        return UNKNOWN_ERROR;
    }

    if (mPackageGroups[0]->packages.size() == 0) {
        printf("idmap: target package has no packages in its first package group, "
                "cannot create idmap\n");
        return UNKNOWN_ERROR;
    }
    ...
    *outSize += 2 * sizeof(uint16_t);
    ...
    for (size_t typeIndex = 0; typeIndex < pg->types.size(); ++typeIndex) {
        ...
        for (size_t entryIndex = 0; entryIndex < typeConfigs->entryCount; ++entryIndex) {
            ...
            if (Res_GETTYPE(overlayResID) + 1 != static_cast<size_t>(typeMap.overlayTypeId)) {
                printf("idmap: can't mix type ids in entry map. Resource 0x%08x maps to 0x%08x"
    ...
    if (map.isEmpty()) {
        printf("idmap: no resources in overlay package present in base package");
        return UNKNOWN_ERROR;
    }
    ...
    *data++ = htodl(overlayCrc);
    ...
    for (int j = 0; j < 2; ++j) {
        ...
        if (I > 255) {
            printf("path exceeds expected 255 characters: %s\n", path);
            return UNKNOWN_ERROR;
        }
        for (size_t i = 0; i < 256; ++i) {
            *p++ = i < I ? path[i] : '\0';
