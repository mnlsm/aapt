...
static std::string formatPrint(const char* source, int flength) {
	std::string result(flength, ' ');
	int source_length = strlen(source);
	int copy_length = source_length > flength ? flength : source_length;
	memcpy((char*)result.data(), source, copy_length);
	return result;
}
...
int calcPercent(long uncompressedLen, long compressedLen)
{
    if (!uncompressedLen) {
		...
		double d_compressedLen = compressedLen;
		double d_uncompressedLen = uncompressedLen;
		return (int)(100.0 - (d_compressedLen * 100.0) / d_uncompressedLen + 0.5);
    }
}
...
int doList(Bundle* bundle)
{
    ...
    if (bundle->getVerbose()) {
		...
		printf("%s%s%s%s%s%s%s%s\n\n",
			formatPrint("Length  ", 12).c_str(),
			formatPrint("Method  ", 12).c_str(),
			formatPrint("Size  ", 12).c_str(),
			formatPrint("Ratio  ", 12).c_str(),
			formatPrint("Offset  ", 20).c_str(),
			formatPrint("Date Time  ", 20).c_str(),
			formatPrint("CRC-32  ", 20).c_str(),
			formatPrint("Name  ", 256).c_str());
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (bundle->getVerbose()) {
			...
			const char* sz_compressionName = compressionName(entry->getCompressionMethod());
			const char* sz_fileName = entry->getFileName() != NULL ? entry->getFileName() : " ";
			char sz_UncompressedLen[12] = { '\0' };
			sprintf(sz_UncompressedLen, "%d", (int)entry->getUncompressedLen());
			char sz_compressedLen[12] = { '\0' };
			sprintf(sz_compressedLen, "%d", (int)entry->getCompressedLen());
			char sz_Percent[12] = { '\0' };
			sprintf(sz_Percent, "0.%d", (int)calcPercent(entry->getUncompressedLen(), entry->getCompressedLen()));
			char sz_LFHOffset[20] = { '\0' };
			sprintf(sz_LFHOffset, "%u", (size_t)entry->getLFHOffset());
			char sz_CRC32[20] = { '\0' };
			sprintf(sz_CRC32, "0x%08x", (size_t)entry->getCRC32());

			printf("%s%s%s%s%s%s%s%s\n",
				formatPrint(sz_UncompressedLen, 12).c_str(),
				formatPrint(sz_compressionName, 12).c_str(),
				formatPrint(sz_compressedLen, 12).c_str(),
				formatPrint(sz_Percent, 12).c_str(),
				formatPrint(sz_LFHOffset, 20).c_str(),
				formatPrint(dateBuf, 20).c_str(),
				formatPrint(sz_CRC32, 20).c_str(),
				formatPrint(sz_fileName, 256).c_str());
        } else {
			printf("%s\n", "unknown entry!");// entry->getFileName());
        }
