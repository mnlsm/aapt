...
static int32_t MapCentralDirectory0(const char* debug_file_name, ZipArchive* archive,
                                    int64_t file_length, int64_t read_amount, uint8_t* scan_buffer) {
  const int64_t search_start = file_length - read_amount;
  ...
  for (; i >= 0; i--) {
    if (scan_buffer[i] == 0x50) {
      ...
      if (*(sig_addr) == EocdRecord::kSignature) {
        break;
      }
    }
  }
  ...
  const int64_t eocd_offset = search_start + i;
  const EocdRecord* eocd = reinterpret_cast<const EocdRecord*>(scan_buffer + i);
  /*
   * Verify that there's no trailing space at the end of the central directory
   * and its comment.
   */
  const int64_t calculated_length = eocd_offset + sizeof(EocdRecord) + eocd->comment_length;
   ...
   */
  if (static_cast<int64_t>(eocd->cd_start_offset) + eocd->cd_size > eocd_offset) {
  ...
  if (!archive->InitializeCentralDirectory(debug_file_name,
                                           static_cast<int64_t>(eocd->cd_start_offset),
...
static int32_t MapCentralDirectory(const char* debug_file_name, ZipArchive* archive) {
  ...
  // 0xffffffff bytes).
  int64_t file_length = archive->mapped_zip.GetFileLength();
  if (file_length == -1) {
    return kInvalidFile;
  }

  if (file_length > static_cast<int64_t>(0xffffffff)) {
    return kInvalidFile;
  }

  if (file_length < static_cast<int64_t>(sizeof(EocdRecord))) {
    return kInvalidFile;
  }
   ...
   */
  int64_t read_amount = kMaxEOCDSearch;
...
static int32_t ParseZipArchive(ZipArchive* archive) {
  ...
  for (uint16_t i = 0; i < num_entries; i++) {
    ...
    const int64_t local_header_offset = cdr->local_file_header_offset;
...
static int32_t FindEntry(const ZipArchive* archive, const int ent, ZipEntry* data) {
  ...
  const int64_t cd_offset = archive->directory_offset;
  ...
  // extra comments.
  const int64_t local_header_offset = cdr->local_file_header_offset;
  if (local_header_offset + static_cast<int64_t>(sizeof(LocalFileHeader)) >= cd_offset) {
    return kInvalidOffset;
  }
  ...
  if (lfh->file_name_length == nameLen) {
    const int64_t name_offset = local_header_offset + sizeof(LocalFileHeader);
  ...
  const int64_t data_offset = local_header_offset + sizeof(LocalFileHeader) +
                              lfh->file_name_length + lfh->extra_field_length;
  if (data_offset > cd_offset) {
    return kInvalidOffset;
  }

  if (static_cast<int64_t>(data_offset + data->compressed_length) > cd_offset) {
    return kInvalidOffset;
  }

  if (data->method == kCompressStored &&
      static_cast<int64_t>(data_offset + data->uncompressed_length) > cd_offset) {
    return kInvalidOffset;
  }
...
class FileWriter : public Writer {
 public:
  ...
  static std::unique_ptr<FileWriter> Create(int fd, const ZipEntry* entry) {
	...
	const int64_t current_offset = _lseeki64(fd, 0, SEEK_CUR);
    if (current_offset == -1) {
      return nullptr;
    }
    ...
    if (!S_ISBLK(sb.st_mode)) {
		...
		HANDLE openfile_handle = (HANDLE)::_get_osfhandle(fd);
		SetFilePointer(openfile_handle, declared_length + current_offset, NULL, FILE_BEGIN);
		if (!::SetEndOfFile(openfile_handle)) {
			result = -1;
		}
        if (result == -1) {
          return std::unique_ptr<FileWriter>(nullptr);
...
static int32_t InflateEntryToWriter(MappedZipFile& mapped_zip, const ZipEntry* entry,
  ...
  do {
    ...
    if (zstream.avail_in == 0) {
      ...
      int64_t offset = entry->offset + (entry->compressed_length - compressed_length);
...
static int32_t CopyEntryToWriter(MappedZipFile& mapped_zip, const ZipEntry* entry, Writer* writer,
  ...
  while (count < length) {
    ...
    int64_t offset = entry->offset + count;
...
int64_t MappedZipFile::GetFileLength() const {
  if (has_fd_) {
	  int64_t result = _lseeki64(fd_, 0, SEEK_END);
    if (result == -1) {
    }
    return result;
  } else {
    ...
    return static_cast<int64_t>(data_length_);
  }
}

// Attempts to read |len| bytes into |buf| at offset |off|.
bool MappedZipFile::ReadAtOffset(uint8_t* buf, size_t len, int64_t off) {
  if (has_fd_) {
    if (!android::base::ReadFullyAtOffset(fd_, buf, len, off)) {
      return false;
    }
  } else {
    if (off < 0 || off > static_cast<int64_t>(data_length_)) {
      return false;
    }
...
void CentralDirectory::Initialize(void* map_base_ptr, int64_t cd_start_offset, size_t cd_size) {
  base_ptr_ = static_cast<uint8_t*>(map_base_ptr) + cd_start_offset;
  length_ = cd_size;
}

bool ZipArchive::InitializeCentralDirectory(const char* debug_file_name, int64_t cd_start_offset,
                                            size_t cd_size) {
  if (mapped_zip.HasFd()) {
    ...
    if (static_cast<int64_t>(cd_start_offset) + static_cast<int64_t>(cd_size) >
